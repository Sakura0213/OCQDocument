...
采购开票该模块用于记录和管理供应商发出的发票信息。在该模块中，您可以新增、删除、查询和修改采购订单的开票信息。您可以填写采购开票的详细信息，包括开票日期、开票金额、开票状态等。在采购订单开票的资料中，您可以查看关联模块的信息，例如供应商、采购订单等。通过采购开票模块，可以将供应商发出的发票与采购订单进行匹配，并验证金额和其他细节的准确性。
...
* 产品信息
  ...
  | 计划完工日期 | 手工录入                                                     |
  | 计划生产数量 | 手工录入                                                     |
  | 实际入库数量 | 在生产入库模块中，属于该生产工单的记录，它们的 < 入库数量 > 字段值的总和 |
  | 入库金额     | 入库单位成本 * 实际入库数量                                  |
  | 领料金额     | 在生产领料模块中，属于该生产工单的记录，它们的 < 领料金额 > 字段值的总和 |
  | 入库单位成本 | ( 领料金额 + 费用总额 ) / 实际入库数量                       |
  | 生产部门     | 部门表                                                       |
  | 费用总额     | 除生产原料成本以外的费用，会参与成本的计算                   |
...
* 产品信息
  ...
  | 领料日期 | 手工录入                             |
  | 领料部门 | 部门表                               |
  | 领料数量 | 下方明细中 < 基本数量 > 字段值的总和 |
  | 领料金额 | 下方明细中 < 成本额 > 字段值的总和   |
...
* 明细
  ...
  | ---------- | ------------------------------------------------------ |
  | 产品名称   | [产品表](#4.3 产品)                                    |
  | 仓库       | 默认由产品携带，为产品表中的 < 默认仓库 > 字段         |
  | 批号       | 手工录入                                               |
  | 仓位       | 由产品携带，为产品在产品表中的 < 平均成本 > 字段       |
  | 包装单位   | 默认由产品携带，为产品表中的 < 包装单位 > 字段         |
  | 数量       | 手工录入                                               |
  | 单位换算率 | 默认由包装单位携带，为单位表中的 < 单位换换算率 > 字段 |
  | 基本数量   | 数量 * 单位换算率                                      |
  | 基本单位   | 手工录入                                               |
  | 成本价     | 默认由产品携带，为产品表中的 < 平均成本 > 字段         |
  | 成本额     | 基本数量 * 成本价                                      |
  | 附注       | 手工录入                                               |
...
* 产品信息
  ...
  | 入库数量   | 手工录入                                                   |
  | 基本单位   | 手工录入                                                   |
  | 单位换算率 | 由包装单位携带，为包装单位在单位表中的 < 单位换算率 > 字段 |
  | 基本数量   | 入库数量 * 单位换算率                                      |
  | 入库成本   |                                                            |
...
* 产品信息
  ...
  | ------------ | ------------------------------------------------------------ |
  | 客户         | [客户表](#7.1 客户)                                          |
  | NO.          | 订单的系统编号 <font color='#F56600'>**\|\|**</font> 系统生成 |
  | 联系人       | 默认由客户携带，为客户表中的 < 联系人 > 字段                 |
  | 联系手机     | 默认由客户携带，为客户表中的 < 联系手机 > 字段               |
  | 类型         | 手工录入                                                     |
  | 日期         | 手工录入                                                     |
  | 附件         | 手工录入                                                     |
  | 图片（合同） | 手工录入                                                     |
  | 总金额       | 下方明细中 < 金额 > 字段值的总和                             |
  | 已收款       | 在回款记录模块中，属于该订单的记录，它们的 < 实收金额 > 字段值的总和 |
  | 未收款       | 总金额 - 已收款 （若总金额 < 已收款，则为 0）                |
  | 已开票       | 在开票模块中，属于该订单的记录，它们的 < 开票金额 > 字段值的总和 |
  | 未开票       | 总金额 - 未开票（若总金额 < 未开票，则为 0）                 |
  | 备注         | 手工录入                                                     |
...
* 明细
  ...
  | 字段       | 说明                                                         |
  | ---------- | ------------------------------------------------------------ |
  | 分类       | 默认由产品携带，为产品表中的 < 分类 > 字段                   |
  | 产品名称   | 「产品表](#4.3 产品)                                         |
  | 规格型号   | 默认由产品携带，为产品表中的 < 规格型号 > 字段               |
  | 数量       | 手工录入                                                     |
  | 包装单位   | 默认由产品携带，为产品表中的 < 包装单位 > 字段               |
  | 单位换算率 | 默认由包装单位携带，为产品表中的 < 单位换算率 > 字段         |
  | 单价       | 默认由产品携带，为产品表中的 < 标准售价 > 字段               |
  | 单位       | 由产品携带，为产品在产品库存表中的 < 基本单位 > 字段         |
  | 折扣       | 手工录入                                                     |
  | 金额       | 单价 \* 数量 \* 单位换算率 * 折扣                            |
  | 已出库     | 在出库记录模块的明细中，属于该条出库记录明细的记录，它们的 < 基本数量 > 字段值的总和 |
  | 未出库     | 数量 - 已出库                                                |
  | 附注       | 默认由产品携带，为产品表中的 < 备注 > 字段                   |
...
* 产品信息
  ...
  | 图片     | 手工录入                                                     |
  | 总金额   | 下方明细中 < 金额 > 字段值的总和                             |
  | 联系电话 | 默认由客户携带，为客户表中的 < 联系手机 > 字段               |
  | 出库成本 | 下方明细中 < 成本额 > 字段值的总和                           |
  | 出库利润 | 总金额 - 出库成本                                            |
  | 已收账款 | 在回款记录模块中，属于该出库记录的记录，它们的 < 实收金额 > 字段值的总和 |
  | 未收账款 | 总金额 - 已收账款                                            |
...
* 明细
  ...
  | 字段       | 说明                                                         |
  | ---------- | ------------------------------------------------------------ |
  | 产品名称   | 默认由订单明细携带，为订单明细的 < 产品名称 > 字段           |
  | 规格型号   | 手工录入                                                     |
  | 仓库       | 默认由产品携带，为产品表中的 < 默认仓库 > 字段               |
  | 批号       | 手工录入                                                     |
  | 仓位       | 手工录入                                                     |
  | 包装单位   | 默认由订单明细携带，为订单明细的 < 包装单位 > 字段           |
  | 数量       | 默认由订单明细携带，为订单明细的 < 待出库 > 字段             |
  | 单价       | 默认由订单明细携带，为订单明细的 < 单价 > 字段               |
  | 单位换算率 | 默认由包装单位携带，为单位表的 < 单位换算率 > 字段           |
  | 基本数量   | 数量 * 单位换算率                                            |
  | 单位       | 默认由订单明细携带，为订单明细的 < 单位 > 字段               |
  | 折扣       | 默认由订单明细携带，为订单明细的 < 折扣 > 字段               |
  | 金额       | 数量 \* 单价 \* 折扣 \* 单位换算率                           |
  | 附注       | 手工录入                                                     |
  | 成本价     | 默认由产品携带，为产品表中的 < 平均成本 > 字段               |
  | 成本额     | 数量 \* 成本价 \* 单位换算率                                 |
  | 订单明细   | 引用关联订单的明细 <font color='#F56600'>**\|\|**</font> 选择关联订单后，系统自动将关联订单中的明细填入 |
...
* 产品信息
  ...
  | 字段       | 说明                                                         |
  | ---------- | ------------------------------------------------------------ |
  | NO         | 销售退货的编号 <font color='#F56600'>**\|\|**</font> 系统生成 |
  | 客户       | [客户表](#7.1 客户)                                          |
  | 关联采购单 | 选择一个属于该客户的采购单                                   |
  | 关联入库   | 选择一个属于该客户的入库单                                   |
  | 退货日期   | 手工录入                                                     |
  | 退货总金额 | 下方明细中 < 退货金额 > 字段值的总和                         |
  | 备注       | 手工录入                                                     |
...
* 明细
  ...
  | 字段       | 说明                                                         |
  | ---------- | ------------------------------------------------------------ |
  | 产品名称   | 默认由出库明细携带，为出库明细的 < 产品名称 > 字段           |
  | 仓库       | 默认由出库明细携带，为出库明细的 < 仓库 > 字段               |
  | 批号       | 默认由出库明细携带，为出库明细的 < 批号 > 字段               |
  | 仓位       | 默认由出库明细携带，为出库明细的 < 仓位 > 字段               |
  | 销售单价   | 默认由出库明细携带，为出库明细的 < 单价 > 字段               |
  | 包装单位   | 默认由出库明细携带，为出库明细的 < 包装单位 > 字段           |
  | 退货数量   | 默认由出库明细携带，为出库明细的 < 数量 > 字段               |
  | 单位换算率 | 默认由出库明细携带，为出库明细的 < 单位换算率 > 字段         |
  | 基本单位   | 默认由出库明细携带，为出库明细的 < 单位 > 字段               |
  | 基本数量   | 退货数量 * 单位换算率                                        |
  | 退货金额   | 基本数量 * 成本单价                                          |
  | 成本单价   | 默认由出库明细携带，为出库明细的 < 成本价 > 字段             |
  | 出库明细   | 引用出库单的明细 <font color='#F56600'>**\|\|**</font> 选择出库单后，系统自动将出库单中的明细填入 |
