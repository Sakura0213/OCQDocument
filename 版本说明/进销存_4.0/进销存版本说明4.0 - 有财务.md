...
该模块用于管理企业的资金账户，包括银行账户、现金账户、支付宝、微信等。在该模块中，您可以新增、删除、查询和修改资金账户的信息，包括账户类别、名称、账号、开户行、余额等。勾选并打开一条资金账户的记录，您可以查看与之关联的其他模块的信息，例如该账户下的收款记录、付款记录等。系统还在记录中提供了快速创建关联模块的选项，以方便用户操作。您还可以在该模块中进行资金的收入和支出操作，记录企业的资金流动情况。该模块可以帮助企业实时掌握资金账户中资金的流动情况，方便进行资金的管理和决策。通过对资金账户的记录和统计，可以更好地进行预算控制、资金调配和财务分析等。
...
* 即 **[产品库存](#cpkc_tz)** 模块，在该模块中，用户可以方便地浏览产品汇总的财务信息。这些信息包括产品的标准售价、加权平均成本、最近采购单价以及总货值等。用户可以通过查看这些数据，更好地了解产品的财务状况和价值。
...
该模块用于管理公司的其他资产。在该模块中，您可以新增、删除、查询和修改其他资产的信息，包括资产名称、类型、单价、数量、折旧期限等。

勾选并打开一条其他资产的记录，您可以查看与之关联的其他模块的信息，例如该资产的折旧情况、使用部门、使用人员等。您还可以进行其他资产的折旧计算、资产处置等操作。

通过该模块，您可以更好地管理公司的其他资产，及时了解资产的情况，合理安排使用和处置，从而提高资产的价值和利用效率。
